...
class Spark_source {
    ...
    function __construct($url)
    {
        ...
        $this->version_data = json_decode(file_get_contents("http://$this->url/api/system/latest", /* use_include_path */ false, $context));
    ...
    function get_spark_detail($spark_name, $version = 'HEAD')
    {
    	...
    	$context = stream_context_create($ctx);
        $json_data = @file_get_contents("http://$this->url/api/packages/$spark_name/versions/$version/spec", /* use_include_path */ false, $context);
    ...
    function search($term)
    {
    	...
    	$context = stream_context_create($ctx);
        $json_data = @file_get_contents("http://$this->url/api/packages/search?q=" . urlencode($term), /* use_include_path */ false, $context); 
        $data = json_decode($json_data);
